...
protocol NewsFeedDataSource {
    func feedItemForIndex(index: IndexPath) -> FeedItem?
    func populateDataSource()
    var count: Int { get }
    var sectionKeys: [Date] { get }
    var groupedFeeds: [Date: [FeedItem]] { get }
}
...
class NewsFeedVM: NewsFeedDataSource {
    ...
    //  Custom getters
    //
    var sectionKeys: [Date] {
        get {
            return datesKeys
        }
    }
    
    var groupedFeeds: [Date : [FeedItem]] {
        get {
    ...
    func feedItemForIndex(index: IndexPath) -> FeedItem? {
        return groupedFeeds[sectionKeys[index.section]]?[index.row]
    }
    
    var dataSource = [FeedItem]()
    var datesKeys = [Date]()
    var groupedDataSource = [Date: [FeedItem]]()
    ...
    func populateDataSource() {
        dataSource = [
            ...
            FeedItem(image: nil, title: "A huge iceberg has drifted close to a village in western Greenland, prompting a partial evacuation in case it splits and the resulting wave swamps homes.",
                ...
                ,
                     feedType: FeedType.feedText, time: Date()),
            FeedItem(image: UIImage(named: Constants().kImgName + "\(5)"), title: "Fifth title", subtitle: "Fifth subtitle", content: "Fifth content", feedType: FeedType.feedImage, time: Date.fromString(stringDate: "2018-06-22 20:11"))
        ]
        groupDatesToSections(sortedData: dataSource)
    }
    ...
    func groupDatesToSections(sortedData: [FeedItem]) {
        ...
        groupedDataSource = sortedData.grouped(by: { (feed: FeedItem) -> Date in
            ...
            let components = calendar.dateComponents(Set<Calendar.Component>([.day,.month,.year,.era]), from: feed.time ?? Date())
            return calendar.date(from: components)!
        })
        
        datesKeys = groupedFeeds.keys.sorted(by: { $0 > $1 })
    }
}
